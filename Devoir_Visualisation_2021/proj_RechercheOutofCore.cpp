...
//
  ...
  */
    ...
    for  (int cur = 20500 ; cur < 55501; cur = cur + 5000){
    std::string p;
        ...
        lut->Build();
        mapper->SetLookupTable(lut);
        mapper->SetScalarRange(cur,cur+5000);
        lut->Build();
       ...
       float *new_rgba = new float[4*winSize*winSize];
        ...
        char namez[128];
        sprintf(namez, "imageDebut_%d_Part%d.png", cur,passNum);
        ...
        char name[128];
        sprintf(name, "final_imageDebut_%d.png", cur);
